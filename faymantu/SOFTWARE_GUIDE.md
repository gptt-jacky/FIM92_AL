...
| `[6]` IO7 | **軟體允許訊號（保險）** | 輸出，由軟體端透過 WebSocket 控制 |
| `[7]` IO8 | **後座力機構狀態** | 1=就緒可觸發 / 0=觸發後冷卻中 |

> **IO7 語義：** IO7 為「軟體端授權保險」，不是直接觸發後座力。IO7=1 期間，後座力觸發權交由硬體端根據三條件（IO7=1 + IO8=1 + IO6+IO2 持續 ≥ 1 秒）自行判定。IO8 於觸發後物理自動歸 0，機構復位後自動回 1，軟體端只需讀取 `io[7]` 即可得知狀態。
...
aim     = io[4]  # IO5  - 瞄準模組
trigger = io[5]  # IO6  - 板機
io7_out = io[6]  # IO7  - 軟體允許訊號/保險（輸出）
io8     = io[7]  # IO8  - 後座力機構狀態（1=就緒, 0=冷卻中）
...
bool io7Out  = io[6] == '1';  // IO7  - 軟體允許訊號/保險（輸出）
bool io8     = io[7] == '1';  // IO8  - 後座力機構狀態（1=就緒, 0=冷卻中）
```
...
| 指令 | 語義 |
|------|------|
| `{"io7":"A1"}` | Tag A（刺針）IO7 → ON（**開保險** — 允許觸發後座力） |
| `{"io7":"A0"}` | Tag A（刺針）IO7 → OFF（**關保險** — 禁止觸發後座力） |
