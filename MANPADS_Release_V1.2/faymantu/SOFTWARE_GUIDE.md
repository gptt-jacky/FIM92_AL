...
----------------------------------------------------------------------
  [A] 允許後座力: OFF  [B] 小震動(IO7): OFF  [C] 大震動(IO8,需+IO7): OFF  |  Ctrl+C to quit
```
...
| 按鍵 | 功能 |
|------|------|
| `A` | 切換 Tag A（刺針）的 IO7 輸出（**允許後座力**） |
| `B` | 切換 Tag B（主射手頭盔震動器）的 IO7 輸出（**小震動**） |
| `C` | 切換 Tag B（主射手頭盔震動器）的 IO8 輸出（配合 IO7 一起 ON 即為**大震動**） |
| `Ctrl+C` | 結束程式 |
...
======================================================================
  ...
  Frame #1600  |  [A] 允許後座力  [B] 小震動(IO7)  [O] 全部 IO7  [1-9] Scene  [Q] Quit
```
...
|------|----------|------|
| `[0]` IO1 | IFF 開關 | 回報給軟體 |
| `[1]` IO2 | 鎖定鍵 | 硬體觸發判定用 + 回報給軟體 |
| `[2]` IOA3 | BCU | 回報給軟體 |
| `[3]` IOA4 | 保險 | 回報給軟體 |
| `[4]` IO5 | 瞄準模組 | 回報給軟體 |
| `[5]` IO6 | 板機 | 硬體觸發判定用 + 回報給軟體 |
| `[6]` IO7 | **允許後座力**（軟體授權保險） | 輸出，由軟體端透過 WebSocket 控制 |
| `[7]` IO8 | **後座力機構就緒** | 1=已就位可觸發 / 0=觸發後冷卻中 |

> **IO7 語義：** IO7 為「允許後座力」的軟體授權訊號，不是直接觸發後座力。IO7=1 期間，後座力觸發權交由硬體端根據三條件（IO7=1 + IO8=1 + IO6+IO2 持續 ≥ 1 秒）自行判定。IO8 於觸發後物理自動歸 0，機構復位後自動回 1，軟體端只需讀取 `io[7]` 即可得知狀態。

**Tag B（主射手頭盔震動器，IO-only）IO 訊號定義：**

| 位置 | 訊號名稱 |
|------|----------|
| `[6]` IO7 | 小震動（輸出，由 Server 控制。單獨 ON = 小震動） |
| `[7]` IO8 | 大震動副訊號（輸出，由 Server 控制。需配合 IO7 同時 ON 才會觸發大震動） |

> Tag B 為 IO-only（無位置追蹤）。  
> **震動邏輯**：IO7=ON 且 IO8=OFF → 小震動；IO7=ON 且 IO8=ON → 大震動；IO7=OFF → 停止（IO8 單獨 ON 無效）。
...
aim     = io[4]  # IO5  - 瞄準模組
trigger = io[5]  # IO6  - 板機
io7_out = io[6]  # IO7  - 允許後座力（輸出，軟體授權保險）
io8     = io[7]  # IO8  - 後座力機構就緒（1=就位, 0=冷卻中）
...
bool io7Out  = io[6] == '1';  // IO7  - 允許後座力（輸出，軟體授權保險）
bool io8     = io[7] == '1';  // IO8  - 後座力機構就緒（1=就位, 0=冷卻中）
```
...
| 指令 | 語義 |
|------|------|
| `{"io7":"A1"}` | Tag A（刺針）IO7 → ON（**允許後座力** — 軟體授權保險開啟） |
| `{"io7":"A0"}` | Tag A（刺針）IO7 → OFF（禁止後座力 — 軟體授權保險關閉） |
| `{"io7":"B1"}` | Tag B（主射手頭盔震動器）IO7 → ON（**小震動**） |
| `{"io7":"B0"}` | Tag B（主射手頭盔震動器）IO7 → OFF（停止震動） |
...
| 指令 | 語義 |
|------|------|
| `{"io8":"B1"}` | Tag B（主射手頭盔震動器）IO8 → ON（配合 IO7 一起 ON 為**大震動**） |
| `{"io8":"B0"}` | Tag B（主射手頭盔震動器）IO8 → OFF |

> **大震動需 IO7 + IO8 同時 ON**。IO8 單獨 ON（IO7=OFF）不會產生震動效果。
