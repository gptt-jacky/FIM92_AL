...
- **IO7 Per-device 控制**：透過鍵盤 `A`/`B` 鍵或 WebSocket 指令，獨立控制各裝置的 IO7
...
│  └──────────────────────────┘             │ (精簡 JSON)     │    程式   │
│                                           └─────────────────┘           │
│  faymantu/RunLocalFile.bat (軟體組本地檔案)                              │
│  ┌──────────────────────────┐                                           │
│  │ TrackingMinimalDemo.exe  │─── build/Release/tracking_data.json       │
│  │ (無 --json)              │    (軟體組可 poll-read 此檔案)             │
│  └──────────────────────────┘                                           │
│                                                                         │
...
│  │ Type/Tag: A │ │ Type/Tag: B │ │ Type/Tag: C │ │ Type/Tag: D │           │
│  │ 刺針        │ │ 頭盔        │ │ 望遠鏡      │ │ 對講機       │           │
│  └──────┬──────┘ └──────┬──────┘ └──────┬──────┘ └──────┬──────┘           │
│         │               │               │               │                  │
│  ┌──────┴──────┐ ┌──────┴──────┐ ┌──────┴──────┐ ┌──────┴──────┐          │
│  │Extension [A]│ │Extension [B]│ │Extension [C]│ │Extension [D]│          │
│  │7 In+IO7 Out │ │7 In+IO7 Out │ │7 In+IO7 Out │ │7 In+IO7 Out │          │
│  └─────────────┘ └─────────────┘ └─────────────┘ └─────────────┘          │
│                         USB / Device Network                            │
...
| **LocalFile** (軟體組) | `faymantu/RunLocalFile.bat` | 500 (檔案) | ~2ms | C++ 直接寫入 `tracking_data.json`，軟體組 poll-read |
...
專為軟體組設計的精簡 WebSocket 資料流。詳細說明見 **[SOFTWARE_GUIDE.md](docs/SOFTWARE_GUIDE.md)**。
...
- 驗證工具：`faymantu/RunWSClient.bat`（互動式輸入 Server IP）
- ~128 msg/sec 推送速率

### 方式二：本地檔案讀取（RunLocalFile.bat）

執行 `faymantu/RunLocalFile.bat` 後，軟體組可直接 poll-read `build/Release/tracking_data.json` 取得即時追蹤資料（C++ 每 ~2ms 原子覆寫）。此模式無需 WebSocket，適合同一台電腦上的整合。
...
軟體組收到的精簡 JSON（DataServer 模式）只保留必要欄位，詳見 [SOFTWARE_GUIDE.md](docs/SOFTWARE_GUIDE.md)。
...
| `trackers[].tag` | string | 裝置代號（A=刺針, B=頭盔, C=望遠鏡, D=對講機，未知類型為空） |
...
> IO7 採 toggle 機制，建議搭配回傳的 `io` 欄位第 7 位（`io[6]`）確認實際狀態。
...
│   ├── RunDataServer.bat         # 軟體組 WebSocket (精簡 JSON)
│   ├── RunWSClient.bat           # WebSocket Client 連線測試
...
| `typeToTag()` | Type → Tag 映射（Type 直接作為 Tag：A/B/C/D） |
...
| `scripts/RunDataServer.bat` | 軟體組 WebSocket（使用 web/data_server.py） |
| `scripts/RunWSClient.bat` | 連線測試：WebSocket Client（互動式輸入 Server IP/Port） |
...
| `faymantu/RunLocalFile.bat` | 軟體組本地檔案模式（C++ 寫入 tracking_data.json，軟體組 poll-read） |
...
> - 軟體組資料介面使用指南請見 [SOFTWARE_GUIDE.md](docs/SOFTWARE_GUIDE.md)
> - 通訊規格書請見 [COMM_SPEC.md](docs/COMM_SPEC.md)
