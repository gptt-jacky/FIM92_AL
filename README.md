...
執行 `RunWithPipe.bat` 後，連接 `ws://localhost:8765` 即可收到完整 JSON（含 scene、stability 等欄位）。附帶 HTTP 伺服器供 3D 網頁使用。

### 方式三：讀取 JSON 檔案（RunTracking.bat）

執行 `RunTracking.bat` 後，C++ 以 ~500Hz 持續覆寫 `build/Release/tracking_data.json`。
C++ 端使用 atomic write（先寫 `.tmp` 再 `rename`）避免讀到不完整資料。
...
│   └── server_ws.py              # 檔案監聽 WebSocket 伺服器 (備用)
│
...
| `RunWithWebSocket.bat` | 備用方案：C++ 寫檔 → server_ws.py → WebSocket + HTTP |
