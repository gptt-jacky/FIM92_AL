...
### 15.5 data_server.py rx_count 已知問題（待修）

在 `stdin_reader()` 中，`transform_fast(line)` 與 `rx_count += 1` 被放在內層 while 迴圈**外面**：

```python
while b"\n" in remainder:      # 內層 loop
    line_bytes, remainder = ...
    line = ...
    if not line.startswith("{"):
        continue
# ← 這兩行在 while 外面，每個 os.read() chunk 只執行一次
transformed = transform_fast(line)
if transformed:
    rx_count += 1
```

**影響**：`rx_fps` 顯示的是 `os.read()` chunk 數，而非實際 JSON 行數。若每個 chunk 含 2 行，顯示值為實際速率的 2 倍。此 bug **尚未修復**，規劃於 V1.3 修正。
