...
## Phase 19：Tag A 採用 Tag G 模式 + 通訊規格書 V1.1 (2026-05-06)

### 19.1 變更背景

Tag G 的 IOA3 PWM 輸出 + IOA4 類比解碼 + Alt Tracker 共存已在 Phase 17/18 完整驗證。
本次正式將 Tag A（刺針本體）切換到相同模式，Tag B（頭盔震動器）停用並保留空殼（刪除線）。
...
| IO1 | 輸入 | IFF 開關 |
| IO2 | 輸入 | 鎖定鍵 |
| IOA3 | PWM 輸出 | 震動器（0=OFF, 1=中震動 55%, 2=強震動 100%） |
| IOA4 | 類比輸入 | 天線/BCU/保險 decode（0–7） |
| IO5 | 輸入 | 瞄準模組 |
| IO6 | 輸入 | 板機 |
| IO7 | 輸出 | 軟體允許後座力訊號 |
| IO8 | 輸入 | 後座力準備就緒 |

IOA4 decode（Tag A 專用，值 3/4 與 Tag G 對調，因電阻網路不同）：
`0=無, 1=BCU, 2=保險, 3=天線, 4=BCU+保險, 5=BCU+天線, 6=保險+天線, 7=BCU+保險+天線`

### 19.3 修改範圍

| 檔案 | 變更 |
|------|------|
| `src/TrackingMinimalDemoCpp.cpp` | HW setup 條件 `A\|\|G` 同路；parsePipeCommand/pipe consumer 新增 ioa3 level 處理 |
| `faymantu/data_server.py` | ws_handler 轉發 `{"ioa3":"A0/A1/A2"}`；移除 Tag B io7/io8 轉發 |
| `faymantu/ws_client.py` | [G] 鍵循環 Tag A IOA3（0→1→2→0）；移除 Tag B 的 B/C 鍵 |
| `faymantu/monitor.py` | IOA4_LEVELS 分 A/G；IOA3* 顯示中震動/強震動（Tag A）；移除 Global IO bar；底部提示更新 |
| `docs/Websocket_刺針感測器通訊系統.md` | V1.0 → V1.1：Tag A IO 表更新、Tag B 刪除線、控制指令重寫（ioa3 指令） |

### 19.4 MSVC 編碼踩坑

C++ struct 欄位的中文 inline comment 在 CP950 codepage 下造成 C2864/C1075（parse error，struct 被視為未結束）。
修正：中文 comment 改為英文（`// for ioa3: 0=off, 1=medium vibe, 2=strong vibe`）。

### 19.5 已知待解問題

- `ws_client.py` [G] 鍵發送 `{"ioa3":"A0/A1/A2"}`，目前實測只能控制 Tag G 的 IOA3，無法控制 Tag A。根因未查，留待下次排查。
