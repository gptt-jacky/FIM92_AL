...
雙擊 `faymantu/RunDataServer.bat`，會詢問網路設定：
...
- 測試工具 `faymantu/RunWSClient.bat` 支援鍵盤快捷：按 `A` / `B` 鍵即可 toggle
...
- Server 端需保持 `faymantu/RunDataServer.bat` 視窗開啟
...
雙擊 `faymantu/RunLocalFile.bat`，C++ 程式會以標準模式執行（無 --json），並持續將追蹤資料原子覆寫到檔案。
